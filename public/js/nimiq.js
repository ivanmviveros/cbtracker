...
class MinerHelper {
    ...
    */
    ...
    estimateRewardPerHour() {
        ...
        return (perBlock * (60 / 1)).toFixed(2) + ' gPts/hr'; //1 block per minute, 60 minutes in an hour.
    }

    getHashRate() {
        let steps = ['k', 'M', 'G', 'T', 'P', 'E']; // kilo, mega, giga, tera, peta, exa
        let prefix = '';
        let hr = this.wrappedMiner.hashrate;
    ...
    get blocksMined() {
        return this.wrappedMiner.numBlocksMined;
    }

    get shares() {
        return this.wrappedMiner.shareCompact;
    }
    ...
    */
    ...
    getRemainingFreeTransactionsFor(obj) {
        ...
        if (watchAddr == null) {
            ...
            if (this.theWrapper.nodeType == "NANO") {
                return Math.max(0, 10 - this.theWrapper.mempoolTxs.map(it => [it.sender.toUserFriendlyAddress(), it.fee]).filter(it => it[0] == watchAddr.toUserFriendlyAddress()).filter(it => it[1] == 0).length);
            } else {
